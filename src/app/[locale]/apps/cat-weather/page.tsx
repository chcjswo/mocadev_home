import type { Metadata } from 'next';
import { getTranslations, setRequestLocale } from 'next-intl/server';
import { AppDetailPage } from '@/components/apps/AppDetailPage';
import type { Locale } from '@/i18n/routing';
import { getAppBase, getAppStructuredData, getBreadcrumbStructuredData } from '@/lib/data/apps';
import { seoConfig } from '@/lib/seo/config';

const base = getAppBase('cat-weather')!;

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: Locale }>;
}): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'seo.apps.cat-weather' });

  const ogImageUrl = base.heroImage.startsWith('http') ? base.heroImage : `${seoConfig.siteUrl}${base.heroImage}`;
  return {
    title: t('title'),
    description: t('description'),
    keywords: t('keywords'),
    robots: { index: true, follow: true },
    openGraph: {
      title: t('title'),
      description: t('description'),
      url: `${seoConfig.siteUrl}/${locale}/apps/cat-weather`,
      siteName: seoConfig.siteName,
      images: [{ url: ogImageUrl, width: 1200, height: 630, alt: t('ogImageAlt') }],
      locale: locale === 'ko' ? 'ko_KR' : 'en_US',
      type: 'website',
    },
    twitter: {
      card: 'summary_large_image',
      title: t('title'),
      description: t('description'),
      images: [ogImageUrl],
    },
    alternates: {
      canonical: `${seoConfig.siteUrl}/${locale}/apps/cat-weather`,
      languages: {
        ko: `${seoConfig.siteUrl}/ko/apps/cat-weather`,
        en: `${seoConfig.siteUrl}/en/apps/cat-weather`,
        'x-default': `${seoConfig.siteUrl}/ko/apps/cat-weather`,
      },
    },
  };
}

export default async function CatWeatherPage({ params }: { params: Promise<{ locale: Locale }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: 'seo.apps.cat-weather' });
  const tApp = await getTranslations({ locale, namespace: 'apps.cat-weather' });
  const tNav = await getTranslations({ locale, namespace: 'nav' });
  const slug = 'cat-weather';
  const appStructured = getAppStructuredData(slug, t('title'), t('description'), base.heroImage, locale);
  const breadcrumb = getBreadcrumbStructuredData(locale, tApp('name'), slug, tNav('home'));

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(appStructured[0].data) }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb.data) }}
      />
      <AppDetailPage slug="cat-weather" />
    </>
  );
}
